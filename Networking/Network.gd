...
func _connected_to_server():
	add_to_player_list()
	# request procedure call is a method that tells other machines to 
	# execute a function
	rpc("_send_player_info", local_player_id, player_data)

# remote -> run this function on every other machine on the network
# but not on the one calling it
